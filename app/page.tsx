...
export default function Home() {
	...
	useEffect(() => {
		...
		const semester = month >= 2 && month <= 7 ? "Genap" : "Ganjil";
		setSemesterText(`${semester} ${shortYear}/${nextYear}`);
	}, []);
				...
				<>
					<div className="flex flex-col gap-2">
						...
						<select
							...
							className="w-full appearance-none rounded-lg border border-zinc-300 bg-white py-2.5 px-3 text-sm font-sans outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 shadow-sm dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100 transition-colors"
						>
							{ids.map((id) => (
					...
					<div className="flex flex-col gap-2">
						...
						<select
							...
							className="w-full appearance-none rounded-lg border border-zinc-300 bg-white py-2.5 px-3 text-sm font-sans outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 shadow-sm dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100 transition-colors disabled:opacity-50"
							disabled={!classes.length}
						>
							<option value="" className="text-zinc-900 dark:text-zinc-100 bg-white dark:bg-zinc-800">Select a class...</option>
							{classes.map((cls) => (
								<option key={cls} value={cls} className="text-zinc-900 dark:text-zinc-100 bg-white dark:bg-zinc-800">
									{cls}
								</option>
							))}
						</select>
										...
										<table className="w-full text-left text-sm whitespace-normal">
											<thead className="bg-zinc-50 dark:bg-zinc-800/50 border-b border-zinc-200 dark:border-zinc-800">
												<tr>
													...
													<th className="py-3 px-3 text-zinc-500 dark:text-zinc-400 font-medium border-r border-zinc-200 dark:border-zinc-800 text-xs uppercase tracking-wider">Name</th>
													<th className="py-3 px-2 text-zinc-500 dark:text-zinc-400 font-medium w-16 text-center text-xs uppercase tracking-wider">ASPRAK Code</th>
												</tr>
											</thead>
